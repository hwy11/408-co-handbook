...
const RICH = {
  "6-1": `
    <div class="card"><h3 class="h3" style="margin-top:0">先建立直觉</h3><p>总线可以理解成计算机内部的公共道路。CPU、主存、I/O 接口都可能要传数据，如果每两个部件之间都单独修一条路，线路数量会爆炸；总线的办法是大家共用一组线，谁要用就先申请，获得使用权后再传输。</p><p>共享带来的好处是结构简单、成本低、扩展方便；坏处是同一时刻不能所有设备随便说话，必须有仲裁和时序规则。第六章短，但它是 CPU、主存、I/O 之间的通信协议基础。</p></div>
  ...
  `,
  "6-2": `
    <div class="card"><h3 class="h3" style="margin-top:0">先建立直觉</h3><p>一次总线传输不是“CPU 把数据扔出去”这么简单。因为总线是共享资源，所以必须先决定谁能用；因为主设备和从设备速度可能不同，所以必须规定什么时候地址有效、什么时候数据有效、什么时候可以撤销信号。这一整套过程叫总线事务和定时。</p></div>
  ...
  `,
  "6-3": `
    <div class="card"><h3 class="h3" style="margin-top:0">本章真正要带走的东西</h3><p>总线的关键词是共享。因为共享，所以需要仲裁；因为设备速度不同，所以需要定时；因为传输能力有限，所以要计算带宽；因为结构不同，所以会有瓶颈和并行性的取舍。</p><p>把总线放回整门课里看，它连接第三章主存、第五章 CPU、第七章 I/O。CPU 取指要走总线，主存读写要走总线，中断响应和 DMA 也离不开总线控制权。它不是孤立小章，而是各部件协同工作的交通规则。</p></div>
  ...
  `,
  "6-4": `
    <div class="card"><h3 class="h3" style="margin-top:0">最容易混的几组概念</h3><p>总线宽度和总线带宽不同。宽度是一次能传多少位，带宽是单位时间能传多少数据。宽度大但频率低，带宽未必高；频率高但每次传输少，带宽也未必高。</p><p>总线周期和时钟周期也不同。时钟周期是一个基础节拍，总线周期是完成一次总线操作需要的时间，可能包含多个时钟周期。机器周期又是 CPU 执行流程里的概念，三者不能随便等同。</p></div>
