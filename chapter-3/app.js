...
const RICH = {
  ...
  `,
  "3-4": `
    <div class="card"><h3 class="h3" style="margin-top:0">先建立直觉</h3><p>外存是“长期仓库”，主存是“工作台”。程序运行时必须把要处理的数据搬到工作台上，但仓库负责便宜、大容量、掉电不丢。磁盘和 SSD 都是外存，但它们慢的原因不一样：磁盘慢在机械定位，SSD 慢在擦写和内部管理。</p></div>
    <h2 class="h2">磁盘访问时间怎么拆</h2><p>磁盘读一个扇区通常经历三段时间：寻道时间、旋转延迟、传输时间。寻道时间是磁头移动到目标磁道的时间；旋转延迟是等目标扇区转到磁头下面的时间，平均约为转一圈时间的一半；传输时间是数据真正经过磁头读出的时间。</p><p>这解释了为什么磁盘随机访问很慢，顺序访问相对高效。随机访问每次都可能重新寻道、等待旋转；顺序访问一旦定位成功，后续扇区可以连续读。408 计算题里给转速 rpm 时，要先换成每转多少秒，再算平均旋转延迟。</p>
    <h2 class="h2">磁盘地址结构</h2><p>磁盘由盘片、盘面、磁道、扇区、柱面组成。同一半径上多个盘面的磁道组成柱面，磁头组切换同柱面不同盘面通常比移动磁臂快。理解这个结构后，才知道为什么操作系统和磁盘调度会尽量减少寻道。</p>
    <h2 class="h2">SSD 和磁盘的差别</h2><p>SSD 没有机械部件，所以随机读性能远好于磁盘。但 Flash 写入前通常要擦除，擦除粒度又比写入粒度大，于是会出现写放大、垃圾回收、磨损均衡等问题。SSD 的快不是“每个操作都没有代价”，而是把机械等待换成了控制器内部的数据搬迁和块管理。</p>
  `,
  "3-5": `
...
const DEEP = {
  ...
  "3-4": {
    ...
    thesis: "外存的核心不是“容量大”，而是用便宜、持久的介质保存大量数据；它慢，是因为定位和写入管理的代价远高于电子存储。",
    visual: "disk",
    anchors: [
      ["3.4.1 磁盘存储器", "磁盘访问时间由寻道时间、旋转延迟、传输时间组成。随机访问慢，主要慢在前两项；顺序访问快，是因为定位成本被摊薄。"],
      ["3.4.2 固态硬盘", "SSD 没有机械寻道，所以随机读快；但 Flash 写入前要擦除，还要做磨损均衡和垃圾回收，所以写性能会受内部管理影响。"]
    ],
    examples: [
      {
        title: "例题 1：平均旋转延迟",
