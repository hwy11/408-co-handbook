...
const RICH = {
  "3-1": `
    <div class="coach-card"><p><strong>存储系统不是“内存知识合集”，它是在给 CPU 找一个现实可行的供货体系。</strong>CPU 像一个非常快的工人，寄存器和 Cache 是手边的小盒子，主存是桌面，外存是仓库。手边越近越快，但越贵越小；仓库越远越慢，但便宜又能装。第三章所有概念，都可以从这个矛盾里长出来。</p></div>
    <div class="memory-economy">
      <div><b>快</b><span>寄存器 / Cache</span><p>离 CPU 近，贵，小，用来救速度。</p></div>
      <div><b>稳</b><span>主存</span><p>程序运行时的工作区，CPU 能直接按地址访问。</p></div>
      <div><b>大</b><span>外存</span><p>长期保存数据，便宜，大，慢。</p></div>
    </div>
  ...
  `,
  "3-3": `
    <div class="coach-card"><p><strong>主存和 CPU 连接题，本质就是给芯片分门牌。</strong>CPU 只会给出一串地址。低位地址负责进芯片里面找第几个格子，高位地址负责决定哪一组芯片应该开门。你只要把这句话吃透，片选、译码、地址分配就不再玄。</p></div>
    <section class="address-board">
      <div class="cpu-box">CPU 地址 A15...A0</div>
      <div class="split-line"><span>高位地址 A15-A14</span><span>低位地址 A13-A0</span></div>
      <div class="route-grid">
        <div><b>片选译码器</b><p>决定第 0/1/2/3 组芯片谁工作</p></div>
        <div><b>芯片地址端</b><p>进入被选芯片内部，定位 16K 个单元中的哪一个</p></div>
      </div>
    </section>
  ...
  `,
  "3-5": `
    <div class="coach-card"><p><strong>Cache 不是“更快的小主存”，而是 CPU 身边的一张小抄。</strong>它赌的是局部性：你刚看过的内容，马上可能还会看；你刚看过的位置，旁边的位置也可能马上会看。Cache 题真正要会的是三件事：主存块放哪、怎么判断在不在、满了换谁。</p></div>
    <section class="cache-address-board">
      <div class="address-word"><span>Tag</span><span>Index / 组号</span><span>Block Offset</span></div>
      <div class="address-explain">
        <p><strong>Offset</strong> 先定位块内第几个字节。</p>
        <p><strong>Index</strong> 决定去 Cache 哪一行或哪一组找。</p>
        <p><strong>Tag</strong> 用来确认“这个位置里放的到底是不是我要的主存块”。</p>
      </div>
    </section>
  ...
  `,
  "3-6": `
    <div class="coach-card"><p><strong>虚拟存储器是在骗程序，但这个骗是善意的。</strong>程序以为自己有一大片连续空间，实际物理主存可能很碎，甚至有些页面根本还在外存里。CPU 发出虚拟地址，硬件先查 TLB 或页表，把虚页号翻译成物理页框号；如果页表说“不在主存”，操作系统才进来处理缺页。</p></div>
    <section class="vm-flow">
      <div><b>虚拟地址</b><span>虚页号 + 页内偏移</span></div>
      <i>查 TLB / 页表</i>
      <div><b>物理地址</b><span>物理页框号 + 原页内偏移</span></div>
      <p>页内偏移不变，变的是“这个虚页现在对应哪个物理页框”。</p>
    </section>
  ...
  `,
  "3-8": `
    <div class="coach-card"><p><strong>第三章最容易丢分，不是因为概念难，而是因为名字像。</strong>Cache miss、TLB miss、缺页都叫“没命中”味儿很重，但它们发生在完全不同的层次，代价也不是一个级别。考试出辨析题，就是在看你脑子里的层次有没有分开。</p></div>
    <section class="confusion-table">
      <div><b>TLB miss</b><p>快表里没有页表项。下一步查页表，页面可能就在主存。</p></div>
      <div><b>Cache miss</b><p>Cache 里没有目标数据块。下一步去主存取块。</p></div>
      <div><b>缺页</b><p>页表显示页面不在主存。下一步操作系统从外存调页。</p></div>
    </section>
